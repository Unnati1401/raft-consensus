...
// Identical to tput.cpp's worker function.
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: ./tput5 <MaxClientCount> <PutRatio>\n";
        return 1;
    }
    ...
    for (int num_clients = 1; num_clients <= max_clients; num_clients *= 2) {
        // *** ONLY CHANGE vs tput.cpp: 5-replica cluster instead of 3 ***
        int num_nodes = 5;
        ...
        std::cout << "Running benchmark with " << num_clients << " client(s) on " << num_nodes << "-replica cluster..." << std::endl;
        ...
        // Aggregate statistics (identical to tput.cpp)
